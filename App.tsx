...
import { Trophy, Users, Clock, MapPin, Plus, AlertTriangle, MessageSquare, Send, Bell, HelpCircle, ListFilter, Check, X, Settings2, Copy, Trash2, LogIn, LogOut, Info, Key, Eye, EyeOff, Cloud, RefreshCw, Share2, WifiOff, Globe, Database } from 'lucide-react';

// Central Database ID for Beach Ivory
...
const App: React.FC = () => {
  ...
  const [syncStatus, setSyncStatus] = useState<'synced' | 'syncing' | 'offline' | 'loading'>('loading');
  ...
  // Cloud Ref Management
  const [blobId, setBlobId] = useState<string | null>(() => {
    return new URLSearchParams(window.location.search).get('bid') || localStorage.getItem('bi_active_blob_id');
  });
  
  const isFirstLoad = useRef(true);
  const activeFine = isEventMode ? FINE_EVENT : FINE_REGULAR;
  ...
  const loadData = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.isEventMode !== undefined) setIsEventMode(data.isEventMode);
  ...
  const saveData = useCallback(async () => {
    ...
    const payload = { players, session: currentSession, isEventMode, lastUpdate: Date.now() };

    try {
      const url = blobId ? `${CLOUD_API}/${blobId}` : CLOUD_API;
      ...
      if (res.ok) {
        if (!blobId) {
          ...
          if (newId) {
            setBlobId(newId);
            localStorage.setItem('bi_active_blob_id', newId);
            const newUrl = new URL(window.location.href);
            newUrl.searchParams.set('bid', newId);
  ...
  }, [players, currentSession, isEventMode, blobId, syncStatus]);
  ...
  useEffect(() => {
    if (!blobId) return;
    const interval = setInterval(() => loadData(blobId), 10000);
  ...
  }, [players, currentSession, isEventMode, saveData]);
  ...
  const handleCopyList = () => {
    const fixList = players.filter(p => p.status === AttendanceStatus.FIX).map((p, i) => `${i + 1}. ${p.name} ${p.paymentStatus === PaymentStatus.PAID ? '✅' : '⏳'}`).join('\n');
    const text = `🏀 BEACH IVORY SESSION 🏀\n🔥 ${currentSession.title}\n📅 ${currentSession.date}\n📍 ${currentSession.location}\n\nFIX LIST:\n${fixList || '- Belum ada -'}\n\nUpdate Realtime:\n${window.location.href}`;
    navigator.clipboard.writeText(text).then(() => alert("List & Link Berhasil Disalin!"));
  };
  ...
  return (
    <div className="min-h-screen pb-40 bg-slate-50 text-slate-900 font-sans">
      ...
      {/* Header Container - Contained to avoid stretching */}
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <h1 className="text-[10px] font-black tracking-widest uppercase opacity-80">Beach Ivory</h1>
                    <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[9px] font-black uppercase border backdrop-blur-md ${syncStatus === 'synced' ? 'bg-emerald-500/20 border-emerald-400/30 text-emerald-100' : 'bg-white/20 border-white/30 text-white'}`}>
                      {syncStatus === 'synced' ? <Database size={10} /> : <RefreshCw size={10} className="animate-spin" />}
                      {syncStatus === 'synced' ? 'Online' : 'Sync...'}
                    </div>
                  </div>
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-start">
                ...
                <div className="flex flex-col gap-2">
                  <button onClick={handleCopyList} className="bg-slate-50 p-3 rounded-2xl border border-slate-100 text-slate-500 hover:bg-slate-100 transition-all shadow-sm"><Share2 size={22} /></button>
              ...
              <div className="flex items-center justify-between border-t border-slate-50 pt-6">
                 <div className="flex items-center gap-4">
                   ...
                   <div className="flex flex-col">
                    <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest mb-0.5 leading-none">Status Slot</span>
      ...
      {/* Floating Toolbar - Conditional for Admins or Brand Presence */}
      {isAdmin && (
